var prefix = appPrefix + "/sys/user";
$(function () {
    //getTreeData();
    load();

});


function formatDateTime(inputTime) {

    var date = new Date(parseInt(inputTime));
    var y = date.getFullYear();
    var m = date.getMonth() + 1;
    m = m < 10 ? ('0' + m) : m;
    var d = date.getDate();
    d = d < 10 ? ('0' + d) : d;
    var h = date.getHours();
    h = h < 10 ? ('0' + h) : h;
    var minute = date.getMinutes();
    var second = date.getSeconds();
    minute = minute < 10 ? ('0' + minute) : minute;
    second = second < 10 ? ('0' + second) : second;
    return y + '-' + m + '-' + d;
};


function load() {
    $('#exampleTable')
        .bootstrapTable(
            {
                method: 'get', // 服务器数据的请求方式 get or post
                url: prefix + "/listAuthUsers", // 服务器数据的加载地址
                showRefresh: false,
                // showToggle : true,
                // showColumns : true,
                iconSize: 'outline',
                toolbar: '#exampleToolbar',
                striped: true, // 设置为true会有隔行变色效果
                dataType: "json", // 服务器返回的数据类型
                pagination: true, // 设置为true会在底部显示分页条
                // queryParamsType : "limit",
                // //设置为limit则会发送符合RESTFull格式的参数
                singleSelect: false, // 设置为true将禁止多选
                cardView: true,
                // contentType : "application/x-www-form-urlencoded",
                // //发送到服务器的数据编码类型
                pageSize: 10, // 如果设置了分页，每页数据条数
                pageNumber: 1, // 如果设置了分布，首页页码
                search: false, // 是否显示搜索框
                showColumns: false, // 是否显示内容下拉框（选择显示的列）
                //sortable:true,
                //sortOrder:'asc',
                sidePagination: "server", // 设置在哪里进行分页，可选值为"client" 或者
                // "server"
                queryParams: function (params) {
                    return {
                        // 说明：传入后台的参数包括offset开始索引，limit步长，sort排序列，order：desc或者,以及所有列的键值对
                        limit: params.limit,
                        offset: params.offset,
                        text: $('#textActivity').val(),
                        deptId: $('#developmentId').val()
                    };
                },
                //table中数据加载完成后执行的函数
                onLoadSuccess: function () {
                    $('.J_menuItem').on('click', function (e) {

                    });
                    return !1
                },
                // //请求服务器数据时，你可以通过重写参数的方式添加一些额外的参数，例如 toolbar 中的参数 如果
                // queryParamsType = 'limit' ,返回参数必须包含
                // limit, offset, search, sort, order 否则, 需要包含:
                // pageSize, pageNumber, searchText, sortName,
                // sortOrder.
                // 返回false将会终止请求
                columns: [
                    {
                        checkbox: true
                    },
                    {
                        field: 'userId', // 列字段名
                        title: '序号', // 列标题
                        width:'5%',
                        switchable: false,
                        formatter: function (value, row, index) {
                            var pageSize = $('#exampleTable').bootstrapTable('getOptions').pageSize;
                            var pageNumber = $('#exampleTable').bootstrapTable('getOptions').pageNumber;
                            return pageSize * (pageNumber - 1) + index + 1;
                        }
                    },
                    {
                        field: 'username',
                        title: '用户名'
                    },
                    //{
                    //    field: 'password',
                    //    title: '密码'
                    //},
                    {
                        class: 'txt',
                        field: 'bankName',
                        title: '银行名称',
              /*          formatter:function(value){
                            return str(value,10);
                        }*/
                    },
                    {
                        class: 'txt',
                        field: 'remark',
                        title: '备注'
                    },
                    {
                        class: 'txt',
                        field: 'devNo',
                        title: '设备编号',
    /*                    formatter:function(value){
                            return str(value,15);
                        }*/
                    },
                    //{
                    //	field : 'uniqueNo',
                    //	title : '唯一编号'
                    //},
                    {
                        field: 'deadline',
                        title: '截止日期',
                        formatter:function(value){
                            if(value){
                                return formatDateTime(value);
                            }else{
                                return "";
                            }
                        }
                    },
                    {
                        field: 'status',
                        title: '状态',
                        align: 'center',
                        formatter: function (value, row, index) {
                            getStatus(value,row.deadline);
         /*                   if (value == '0') {
                                return '<span class="label label-danger">禁用</span>';
                            } else if (value == '1') {
                                return '<span class="label label-primary">正常</span>';
                            }*/
                        }
                    },
                    {
                        title: '操作',
                        field: 'id',
                        align: 'center',
                        formatter: function (value, row, index) {
                            var c = '<a  class="btn btn-primary btn-sm ' + s_check_h + '" href="#" mce_href="#" title="用户详情" ' +
                                'onclick="edit(\'' + row.userId + '\',\'1\')"><i class="fa fa-info "></i></a> ';
                            var e = '<a  class="btn btn-primary btn-sm ' + s_edit_h + '" href="#" mce_href="#" title="编辑用户" ' +
                                'onclick="edit(\'' + row.userId + '\',\'2\')"><i class="fa fa-edit "></i></a> ';
                            var d = '<a class="btn btn-danger btn-sm ' + s_remove_h + '" href="#" title="删除"  mce_href="#" onclick="remove(\''
                                + row.userId
                                + '\')"><i class="fa fa-remove"></i></a> ';
                            var f = '<a class="btn btn-success btn-sm ' + s_resetPwd_h + '" href="#" title="重置密码"  mce_href="#" onclick="resetPwd(\''
                                + row.userId
                                + '\')"><i class="fa fa-key"></i></a> ';
                            var g = '<a class="btn btn-success btn-sm J_menuItem' + s_deviceContent_h + '" href="/system/deviceContent/' + row.userId + '" ' +
                                'title="内容管理"  mce_href="#" onclick=""><i class="fa fa-map"></i><strong> 内容管理</strong></a> ';
                            return c + e + d + f ;
                        }
                    }]
            });
}

function getStatus(value,time){
    if(time){
        var userTime = time + 24*3600*1000 - 1;
        var now = new Date().getTime();
        if(userTime < now){
            return '<span class="label label-warning">过期</span>';
        }else{
            if (value == '0') {
                return '<span class="label label-danger">禁用</span>';
            } else if (value == '1') {
                return '<span class="label label-primary">正常</span>';
            }
            return "";
        }

    }else{
        return "";
    }



}



function str(text,len){
    if(text){
        if(text.length < len){
            return text;
        }else{
            return text.slice(0,len) + "...";
        }
    }else{
        return "";
    }
}

function reLoad() {
    var params = {
        query: {
            deptId: $("#developmentId").val(),
            offset:0
        }
    };
    $('#exampleTable').bootstrapTable('refresh', params);
    $('#exampleTable').bootstrapTable('refreshOptions',{pageNumber:1});
}
function add() {
    // iframe层
    layer.open({
        type: 2,
        title: '增加用户',
        maxmin: false,
        shadeClose: false, // 点击遮罩关闭层
        area: ['800px', '520px'],
        content: prefix + '/add',
    });
}
function remove(id) {
    layer.confirm('确定要删除选中的记录？', {
        btn: ['确定', '取消']
    }, function () {
        $.ajax({
            url: prefix + "/remove",
            type: "post",
            data: {
                'id': id
            },
            success: function (r) {
                if (r.code == 0) {
                    layer.msg(r.msg);
                    reLoad();
                } else {
                    layer.msg(r.msg);
                }
            }
        });
    })
}
function edit(id, type) {
    //type:1为查看详情；2为编辑
    var title = '';
    if (type == '1') {
        title = '用户详情';
    } else if (type == '2') {
        title = '用户修改';
    }
    layer.open({
        type: 2,
        title: title,
        maxmin: true,
        shadeClose: false,
        area: ['800px', '520px'],
        content: prefix + '/edit/' + id + '/' + type // iframe的url
    });
}
function resetPwd(id) {
    layer.open({
        type: 2,
        title: '重置密码',
        maxmin: true,
        shadeClose: false, // 点击遮罩关闭层
        area: ['400px', '260px'],
        content: prefix + '/resetPwd/' + id // iframe的url
    });
}
function batchRemove() {
    var rows = $('#exampleTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
    if (rows.length == 0) {
        layer.msg("请选择要删除的数据");
        return;
    }
    layer.confirm("确认要删除选中的'" + rows.length + "'条数据吗?", {
        btn: ['确定', '取消']
        // 按钮
    }, function () {
        var ids = new Array();
        // 遍历所有选择的行数据，取每条数据对应的ID
        $.each(rows, function (i, row) {
            ids[i] = row['userId'];
        });
        $.ajax({
            type: 'POST',
            data: {
                "ids": ids
            },
            url: prefix + '/batchRemove',
            success: function (r) {
                if (r.code == 0) {
                    layer.msg(r.msg);
                    reLoad();
                } else {
                    layer.msg(r.msg);
                }
            }
        });
    }, function () {
    });
}

var openDept = function(){
    layer.open({
        type:2,
        title:"选择部门",
        area : [ '300px', '450px' ],
        content:appPrefix + "/system/sysDept/treeView/0"
    })
};

function loadDept( deptId,deptName){
    $("#developmentId").val(deptId);
    $("#cityName").val(deptName);
    reLoad();
}

function searchActivity(){
    var textActivity = $("#textActivity").val();
    var params = {
        query: {
            deptId: $("#developmentId").val(),
            offset:0,
            text:textActivity
        }
    };
    $('#exampleTable').bootstrapTable('refresh', params);
    $('#exampleTable').bootstrapTable('refreshOptions',{pageNumber:1});
}

