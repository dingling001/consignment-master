...
function setImgValue(){
    base_1 = $("#goodsImgUrl").attr("src");
    base_2 = $("#goodsDetailUrl").attr("src");
    var li = $("#imgBox").find("li");
    base_3 = li.eq(0).find("img").attr("src");
    base_4 = li.eq(1).find("img").attr("src");
    base_5 = li.eq(2).find("img").attr("src");
}

function  checkImgUrl(){
    var url = $("#goodsImgUrl").attr("src");
    if(!url){
        $("#goodsImgUrl").hide();
    }
    var url1 = $("#goodsDetailUrl").attr("src");
    if(!url1){
        $("#goodsDetailUrl").hide();
    }

    var inner = $("#goodsCarousel").val();
    var ul = '';
    if(inner){
        var list = inner.split(",");
        if(list.length > 0){
            for(var i = 0; i < list.length; i ++){
                var li = '<li><img src="' + list[i] + '" data-index="' + (i + 2) + '" class="layui-upload-img shop-img float-left"></li>';
                ul += li;
            }
        }
    }
